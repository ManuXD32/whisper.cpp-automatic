...
# Preguntar al usuario si quiere dejar el idioma original o traducirlo
translate_language = input("¿Quieres traducir el texto? (Sí/No): ")
if translate_language.lower() == 'sí' or translate_language.lower() == 'si':
    target_language = input("¿A qué idioma quieres traducir el texto? (por ejemplo, 'es' para español, 'en' para inglés, etc.): ")
    option = f'-l {target_language}'
else:
    option = '-l auto'
...
for video_file in video_files:
    ...
    transcribe_cmd = f'./main {option} --output-file "{output_path}" -m "{model_path}" "{audio_path}" -t {num_cores}'
    subprocess.call(transcribe_cmd, shell=True)
